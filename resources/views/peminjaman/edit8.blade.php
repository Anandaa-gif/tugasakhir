            ...
            <div class="col-md-8">
                <div class="card shadow-lg border-0 rounded-3">
                    <div class="card-header bg-gradient-primary text-white rounded-top-3">
                        <h4 class="mb-0 fw-semibold"><i class="fas fa-edit me-2"></i>Edit Peminjaman</h4>
                    </div>
                    <div class="card-body p-4">
                        <form action="{{ route('peminjaman.update8', $peminjaman->id) }}" method="POST"
                            ...
                            <div class="mb-3">
                                ...
                                <select name="anggota_id" id="anggota_id"
                                    class="form-select form-select-lg select2 @error('anggota_id') is-invalid @enderror"
                                    required>
                                    <option value="">-- Pilih Anggota --</option>
                                    @foreach ($anggota as $a)
                                        <option value="{{ $a->id }}"
                                            {{ old('anggota_id', $peminjaman->anggota_id) == $a->id ? 'selected' : '' }}>
                                            {{ $a->namalengkap ?? 'Nama Tidak Diketahui' }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('anggota_id')
                            ...
                            {{-- Buku (Multiple Select) --}}
                            <div class="mb-3">
                                <label for="buku_id" class="form-label fw-medium">Judul Buku (maksimal 2)</label>
                                <select name="buku_id[]" id="buku_id"
                                    class="form-select form-select-lg select2 @error('buku_id') is-invalid @enderror"
                                    multiple required>
                                    @foreach ($buku as $b)
                                        <option value="{{ $b->id }}"
                                            {{ in_array($b->id, old('buku_id', $selectedBukuIds ?? [$peminjaman->buku_id])) ? 'selected' : '' }}>
                                            {{ $b->judul_buku }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" name="tanggal_kembali" id="tanggal_kembali"
                                    ...
                                    class="form-control form-control-lg @error('tanggal_kembali') is-invalid @enderror">
